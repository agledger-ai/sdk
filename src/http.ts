...
export class HttpClient {
  ...
  /**
   * Binary-body request with the same retry / idempotency / abort semantics as
   * the JSON path. Sends `body` (a Uint8Array) with `contentType` and reads
   * the response as `Uint8Array` (no JSON parsing). Use for SCITT
   * `application/cose` and `application/cose-sequence` flows.
   *
   * Errors: when the API returns a 4xx/5xx, the response body is captured raw
   * on `AgledgerApiError.rawBody` (SCITT errors are
   * `application/concise-problem-details+cbor` per RFC 9290, not JSON).
   */
  async requestBinary(
    method: 'GET' | 'POST',
    path: string,
