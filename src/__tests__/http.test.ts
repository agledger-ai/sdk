...
describe('HttpClient', () => {
  ...
  describe('error mapping', () => {
    ...
    it('forwards top-level extension fields (recoveryHint, refreshUrl, missingScopes) from the body', async () => {
      // Regression: mapError once whitelisted error fields and dropped these
      // top-level extensions, so the error classes (which read them straight
      // off the body) silently surfaced undefined. Found via live integration
      // testing against a 403 PLATFORM_REQUIRED that carries a recoveryHint.
      const fetch = mockFetch({
        ok: false,
        status: 403,
        json: {
      ...
      try {
        ...
        const pe = err as PermissionError;
        expect(pe).toBeInstanceOf(PermissionError);
        expect(pe.recoveryHint).toBe('Use a platform key or ask your operator.');
        expect(pe.refreshUrl).toBe('/v1/auth/me');
        expect(pe.missingScopes).toEqual(['admin:system']);
      }
    });
    ...
    it('falls back to RFC 9457 detail/title when message is absent', async () => {
      const fetch = mockFetch({
        ok: false,
        status: 403,
        json: { type: '/problems/forbidden', title: 'Forbidden', detail: 'No access to this org' },
      });
      const client = createClient(fetch);
      try {
        await client.get('/test');
        ...
        expect((err as PermissionError).message).toBe('No access to this org');
      }
    });
