...
describe('critical routes exist in the current API spec', () => {
  const CRITICAL_ROUTES: Array<[string, string]> = [
    ...
    ['GET', '/v1/admin/trusted-issuers/{id}'],
    ['PATCH', '/v1/admin/trusted-issuers/{id}'],
    ['DELETE', '/v1/admin/trusted-issuers/{id}'],
    ['POST', '/v1/admin/trusted-issuers/{id}/revoke-certs'],
    ['POST', '/v1/admin/ephemeral-certs/{id}/revoke'],
    ['GET', '/v1/admin/ops-summary'],
    ['GET', '/v1/admin/vault/scan'],
    ['GET', '/v1/peer-agents'],
...
describe('body-field contracts on critical POST routes', () => {
  ...
  it('POST /v1/records/{id}/verdict requires completionId and verdict', () => {
    const route = routeMap.get('POST /v1/records/{id}/verdict');
    expect(route).toBeDefined();
    expect(route!.requiredFields).toEqual(expect.arrayContaining(['completionId', 'verdict']));
  });
  ...
  it('POST /v1/admin/records/import requires orgId, source, records', () => {
    const route = routeMap.get('POST /v1/admin/records/import');
    expect(route).toBeDefined();
    expect(route!.requiredFields).toEqual(expect.arrayContaining(['orgId', 'source', 'records']));
  });
});
