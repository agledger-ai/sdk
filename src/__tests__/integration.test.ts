...
describe('SDK integration: response shape validation', async () => {
  ...
  it('schemas.get() returns schema with recordSchema + completionSchema', async () => {
    const schema = await client.schemas.get('notarize-generic-v1');
    expect(schema.type).toBe('notarize-generic-v1');
    expect(schema.recordSchema).toBeDefined();
    expect(schema.completionSchema).toBeDefined();
  });
  ...
  it('record lifecycle: create → get → cancel', async () => {
    ...
    const record = await client.records.create({
      ...
      criteria: { summary: 'integration-test lifecycle record' },
    });
    createdRecordId = record.id;
    expect(typeof record.id).toBe('string');
    expect(record.type).toBe('principal-gate-generic-v1');
    expect(record.status).toBe('CREATED');
    expect(typeof record.createdAt).toBe('string');
    expect(record.signedStatement).toBeDefined();
  ...
  // --- Completions (agent-side) ---

  it('record + completion lifecycle: create → submit completion → get completion', async () => {
    if (!AGENT_KEY) return;
    ...
    const record = await agentClient.records.create({
      ...
      contractVersion: '1',
      platform: 'integration-test',
      criteria: { summary: 'integration-test completion flow' },
      autoActivate: true,
    });
    createdRecordId = record.id;

    // Submit completion
    const completion = await agentClient.completions.submit(record.id, {
      evidence: { summary: 'query result', deliverableUrl: 'https://example.com/report' },
    });
    expect(typeof completion.id).toBe('string');
    expect(completion.recordId).toBe(record.id);

    // Get completion
    const fetched = await agentClient.completions.get(record.id, completion.id);
    expect(fetched.id).toBe(completion.id);

    // List completions
    const page = await agentClient.completions.list(record.id);
    assertPage(page, 'completions.list');
  ...
  it('discovery.getConformance() returns capabilities + version', async () => {
    const conf = await client.discovery.getConformance();
