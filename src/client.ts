...
import { ScittResource } from './resources/scitt.js';
import { PredicatesResource } from './resources/predicates.js';
...
export class AgledgerClient {
  ...
  constructor(options: AgledgerClientOptions) {
    ...
    this.scitt = new ScittResource(http);
    this.predicates = new PredicatesResource(http);
  }
}

/**
 * Create a lightweight federation client for peer servers that authenticate
 * via a bearer token (typically issued during the peer handshake on
 * `POST /federation/v1/peer`).
 *
 * @example
 * ```ts
 * const fc = createFederationClient({ bearerToken: 'bt_…' });
 * await fc.federation.submitStateTransition({
 *   recordId, state: 'FULFILLED', type: 'notarize-generic-v1', idempotencyKey: 'idem-1',
 * });
 * ```
 */
